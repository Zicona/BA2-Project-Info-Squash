...
abstract class SpecialObject (x: Float, y: Float, diametre: Float, var isGentle : Boolean){
    ...
    fun returnNormal(view: DrawingView, lesBonus: ArrayList<SpecialObject>) {
        when(this) {
            is SizeModifier -> {
                if (mustChangeBonus) {
                    if (isGentle) view.balle.changeTaille(3)
                    mustChangeBonus = false
                } else if (mustChangeMalus) {
                    view.balle.changeTaille(4)
                    mustChangeMalus = false
                }
            }
            is DoublePoints -> {for (p in view.lesParois) p.doublePoint = byPass}
            is SafeWall -> {view.lesParois[2].onScreen = byPass}
            is Raquette -> {view.canClickBall = true}
        }
    }

    open fun Activate(b: Balle, view: DrawingView) {
        if (isGentle) view.playBonusSound() else view.playMalusSound()
